  ...
  return (
    <div>
      ...
      <div className='mt-1 flex items-start gap-1.5'>
        <InformationCircleIcon className='w-[18px] h-[18px] text-primary-500 dark:text-primary-400 flex-shrink-0 mt-0.5' />
        <p className='text-sm leading-5 text-primary-600 dark:text-primary-400'>
          {t('questions.onboarding_hint', { defaultValue: 'Your answers help us tailor advice to your background and goals.' })}
        </p>
      </div>
    </div>
  )
...
export const WelcomeCard: FC = () => {
  ...
  return (
    <div className='rounded-2xl border border-primary-100 bg-gradient-to-br from-primary-50 to-primary-100 px-4 py-4 text-gray-800 shadow-sm dark:border-primary-900/40 dark:from-primary-950/40 dark:to-primary-900/30 dark:text-gray-100'>
      <div className='text-base font-semibold text-primary-900 dark:text-primary-200'>
      ...
      <div className='mt-3 rounded-xl border border-primary-200 bg-primary-50 px-3 py-3 dark:border-primary-700/40 dark:bg-gray-800/50'>
  ...
  return (
    <div className={cn('flex items-center gap-3', className)}>
      <Button
        ...
        disabled={disabled}
      >
        <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-2" style={{ width: 'var(--start-chat-icon-size, 20px)', height: 'var(--start-chat-icon-size, 20px)' }}>
      ...
      {disabled && (
        <span className="max-w-[120px] text-sm leading-tight text-amber-600 md:max-w-none md:whitespace-nowrap">{t('questions.actions.form_incomplete_hint')}</span>
      )}
    </div>
  )
}
...
export const EditBtn = ({ className, onClick }: { className?: string, onClick: () => void }) => {
  ...
  return (
    <div
      className={cn('px-2 flex space-x-1 items-center rounded-md cursor-pointer text-primary-600 dark:text-white', className)}
      onClick={onClick}
    >
